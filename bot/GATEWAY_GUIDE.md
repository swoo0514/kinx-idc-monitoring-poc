...
- **분류**: `classify(alert_name, item_key)` 가 알림을 `replication` / `cpu_io_pressure` /
  ...
  - 그 경계를 `\w`가 아니라 **영숫자**로 잡는다(`(?<![A-Za-z0-9])`). `\w`는 밑줄을 단어 문자로
    보므로 `sshd`가 `sshd_config`에 걸리지 않았다(실측). 영숫자로 바꿔도 원래 목적은 유지된다 —
    scan의 `sca`, room의 `oom`, shutdown의 `down`은 앞뒤가 영문자라 여전히 차단된다.
...
### 무엇을 물어보는가 (`holmes.build_question`)

**headline("N건이 1개 사건 · host")만 넘기면 안 된다.** 그러면 홈즈가 무슨 사건인지 모른 채
그 호스트에서 **그 순간 활성인 아무 문제나** 조사한다(2026-07-31 실측). 알림 이름·유형·관측창을
질문에 실어야 우리가 병합한 그 사건을 조사한다.

### 결과를 어디로 돌려주는가

심층조사는 **별개 알림을 만들지 않는다.** 사건 하나가 피드에 한 행으로 유지돼야 하기 때문이다.

| 경로 | 어디에 | 왜 |
|---|---|---|
| 읽기 | 봇 초동분석이 올라간 **Slack 스레드의 답글** | 사람이 사건을 따라 읽는 자리 |
| 실행 화면 | 원래 **Keep 알림의 Note**(`fingerprint` 매칭) | 관제 화면에서 사건 1행 유지 |

3분 수준이라 30초 예산과 분리해 **fire-and-forget**으로 돌린다. 승인이 아니라 읽기 전용이므로
자동 발동해도 무방하다(읽기=자동 / 쓰기=승인 규칙).

> **fire-and-forget 태스크는 강참조를 유지해야 한다.** 파이썬 공식 문서가
> `asyncio.create_task`에 대해 "save a reference ... may be garbage collected"라고 명시한다.
> 참조를 안 잡으면 조사가 **중간에 조용히 사라진다.** `triage._bg_tasks` 가 그 참조 집합이고,
> 완료 콜백에서 정리하며 예외를 로깅한다.
