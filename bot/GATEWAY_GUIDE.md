...
**로그 축 면제 대상:** 인증서 만료 감시 대상이나 리포트 값 저장용 가상 호스트와 같이 OS 및 에이전트가 존재하지 않는 호스트는 로그·보안 데이터가 원천적으로 발생하지 않습니다. 이러한 호스트를 이름 불일치로 판정하면 알림 발생 시마다 분석이 구동되며, 실제 불일치 대응에 사용할 시간당 상한(§8-3)을 선점하게 됩니다. `LOG_AXIS_EXEMPT_HOSTS` 에 Zabbix 호스트명 패턴을 등록하면 해당 호스트는 조회를 수행하지 않고 `disabled` 로 기록합니다. 와일드카드는 `*` 를 사용합니다.

```bash
LOG_AXIS_EXEMPT_HOSTS=cert-*,report-*
```
