...
| 알림이 들어오지 않음 | `gateway.alive=1` 이고 `since_last_alert > 1800` | Average | 게이트웨이 정상, 발행 측 유입 중단 의심 |
...
**도입 배경:** 관측 소스 3종이 동일 호스트를 각기 다른 이름으로 식별하는 문제가 랩 환경에서 실측되었습니다(Zabbix `node1` / Loki `vm-target-001.novalocal` / Wazuh FQDN). 이름이 일치하지 않으면 API 는 정상 응답과 함께 빈 결과를 반환하므로, 기존 로직은 이를 조회 성공으로 처리하여 분석 봇이 "로그에 기록 없음"을 사실로 단정하는 결함이 존재하였습니다.
