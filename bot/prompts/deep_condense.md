...
받는 것은 소목표 하나, 조사 목표 한 줄, 그 축의 결과 둘(사건 구간·정상 구간)이다. 사건의
배경도 다른 축도 모른다. 모르는 것을 짐작해 채우지 말라.
...
- `evidence` 는 **원문 그대로** 옮긴다. 요약하지 말고 계산하지 말라. 뒤에서 사람이 이 줄로
  되짚는다.
- 계열이나 줄이 여럿이면 **`goal` 에 적힌 것과 이름이 맞닿은 것을 먼저 남긴다.** 복제 지연을
  조사하는데 메모리와 버퍼풀만 남기면 뒤에서 원인을 짚을 재료가 사라진다. 그다음으로
  **정상 구간 대비 가장 크게 달라진 것**을 남긴다. 변화가 없는 계열은 버려도 된다.
