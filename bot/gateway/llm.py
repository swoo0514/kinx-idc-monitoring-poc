...
- **logs 는 창의 전부가 아니라 고른 것이다.** 각 항목의 `why` 가 뽑힌 이유다 —
  `error`(등급), `novel`(경고·평소와 다름), `pre`(첫 오류 직전 문맥), `recent`(최신),
  `all`(창이 작아 전부 실림). `n` 은 **같은 모양의 줄이 창에 몇 개 있었는지**이며, 실린
  것은 그중 일부다. `n` 이 크면 그 줄은 한 번이 아니라 반복된 것이므로 빈도로 읽어라.
  `t` 는 초 단위 시각이다. 항목 사이 시각이 크게 벌어져 있으면 그 사이 줄은 안 실린
  것이니 이어진 사건으로 읽지 마라.
