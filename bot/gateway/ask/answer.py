...
# 그림 표시. 화면은 그림을 따로 그리므로 본문에 남으면 지저분한 글자일 뿐이다.
...
def render_answer(args: dict, had_evidence: bool = True) -> str:
    """답 도구의 필드를 사람이 읽는 글로. 표시와 구간을 산문에서 뺀 대가로 여기서 만든다."""
    a = args or {}
...
def strip_handles(text: str) -> str:
    """답에서 그림 표시를 걷어 낸다. 앞뒤 공백도 정리한다."""
    # 마크다운 그림 표기를 통째로 걷어 낸다 — 그림은 따로 붙는다
    out = _MD_IMAGE_RE.sub("", str(text or ""))
    out = _HANDLE_RE.sub("", out)
    # 표시가 괄호 안에 있었으면 빈 괄호가 남는다
