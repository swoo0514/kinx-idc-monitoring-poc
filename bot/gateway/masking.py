...
class Masker:
    ...
    def __init__(self, token_fn=None):
        ...
        self._re = None
        self._lower = {}
        # 토큰을 만드는 방법 — 기본은 등록 순서. 여러 턴 대화는 이름에서 산출하는 함수를 준다
...
def cannot_mask(masker=None) -> bool:
    ...
    # 이번 요청의 마스커가 이름을 들고 있으면 가릴 수 있다 — 전역 표가 비어도 된다
...
def _build_incident_context(context: dict, sev: str, masker: Masker) -> dict:
    ...
    for a in context.get("alerts", []) or []:
        ...
        alerts.append({
            ...
            # 수집기가 실패를 표시해도 화이트리스트에 없으면 안 실려 "지표 이상 없음"이 된다
            "error": a.get("error"),
        })
    ...
    return {
        "incident": {
            ...
            # 계약 제약과 자동 조치 가능 여부 — 값이 식별자가 아니라 라벨이라 마스킹하지 않는다
