...
async def fetch_zabbix(days: int) -> list:
    ...
    async with httpx.AsyncClient(verify=False) as client:
        events = await paged(client, {
            ...
            #
            # 실측 2026-08-10 (--diag 계단식 진단, 사내 Zabbix 7.0.27):
...
def overlap_counts(events: list, axis: str, min_open_s: int = 0):
    ...
    # 실측 2026-08-10: 통과 조합의 81%가 단 한 번의 사건에서 나왔다(수백 대 규모 호스트가
    # 몇 분 안에 함께 발화한 건).
    ...
    return {"pairs": pairs, "n_axis": n_axis,
            ...
            # 일수만 보면 "겹침 252건이 4일에 몰린" 형태가 안 걸러진다 — 실측(2026-08-10)
...
def permute_overlap(events: list, observed: dict, axis: str, rounds: int,
    ...
    유의 판정을 받는다. 실측 2026-08-10: 검정 2,344개 중 **1,997개(85%)가 통과** —
    결과가 아니라 고장이다.
