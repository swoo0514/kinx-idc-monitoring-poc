...
def template(name: str) -> str:
    """트리거명에서 변수부를 지워 '유형'으로 접는다.

    왜 필요한가 — 실측(2026-08-07, 사내 90일). 원본 트리거명은 **고유율 95%** 였다. 이름에 인터페이스 번호·임계값·측정값이 그대로 박혀 있어 사실상
    이벤트마다 다른 이름이 되기 때문이다. 그 상태로는 같은 이름이 두 번 나오지 않으므로
    마이닝 축이 성립하지 않는다.

    **숫자만 치환하면 380종으로 접힌다**(고유율 0.9%). 그중 5회 이상 등장하는 65개가
    이벤트의 98.9%를 덮는다. IP·괄호까지 추가로 치환해도 380 으로 동일했으므로 숫자
    치환이 결정적이다. IP 는 표기 안정성을 위해 남겨 둔다.

    로그 템플릿 마이너(Drain3)와 같은 목적이지만, 알림명은 로그 본문과 달리 구조가
    단순해 정규식으로 충분하다는 것이 위 실측의 결론이다.
    """
...
async def fetch_zabbix(days: int) -> list:
    ...
    async def paged(client, params, label, chunk_days=10):
        ...
        왜 — 90일치를 한 번에 요청하면 서버가 500 을 낸다(실측 2026-08-07). 계단식
        진단상 limit 20000 + 최소 필드까지는 통과하고 extend 를 얹으면 깨졌으므로,
        임계는 건수가 아니라 **응답 크기**다. 호스트·태그를 함께 받는 실제 수집 쿼리는
        그 임계를 넘는다. 어느 값이 한계인지 맞히는 대신 기간을 잘라 확실히 통과시킨다.

        한 조각이 실패해도 나머지는 살린다. 다만 조용히 넘기면 그 기간에 사건이 없었던
        것으로 오독되므로 반드시 남긴다.
        """
        out, gaps = [], []
    ...
    async with httpx.AsyncClient(verify=False) as client:
        events = await paged(client, {
            # objectid = 트리거 ID. 해소 시각을 짝짓는 데 쓴다.
            #
            # 실측 2026-08-07 (--diag 계단식 진단, 사내 Zabbix 7.0.27):
            #   limit 20000 + 최소 필드      → 통과
            #   limit 20000 + output=extend → **500**
            #   limit 1 + r_eventid          → 통과 (지정 자체는 가능)
            # 즉 문제는 필드 이름이 아니라 **대량 조회에서의 응답 크기**다. r_eventid 는
            # event_recovery 테이블 조인이라 대량 조회에서 같은 위험이 있어 쓰지 않는다.
            # 대신 OK 이벤트를 따로 조회해 같은 트리거의 다음 해소를 찾는다.
            "output": ["eventid", "name", "clock", "severity", "objectid"],
            "selectHosts": ["host"],
            "selectTags": "extend",
            "value": 1,
...
def apply_exclusions(events: list, name_pats: list, host_pats: list) -> list:
    """만성 노이즈를 걷어낸 뒤 무엇이 보이는지 확인하기 위한 필터.

    왜 필요한가 — 실측(2026-08-07, 사내 90일). 인터페이스 오류 계열 한 종류가 전체
    이벤트의 94.0%, 호스트 2대가 74.6%를 차지한다. 이 상태에서는 유의 판정된 조합의
    대부분이 그 계열 내부 조합이라 **다른 증상 간 관계가 결과에서 밀려난다.**
    계열을 빼고 재측정하면 서로 다른 증상 간 조합으로 결과가 바뀐다.

    이 옵션은 데이터를 고치는 것이 아니라 **정비했을 때 무엇이 보이는지 미리 보는 것**이다.
...
def lag_counts(events: list, axis: str, lag_max: int, nbins: int):
    ...
    고정 창을 대체한다. 왜 대체가 필요한가 — 실측(2026-08-07, 강도를 아는 조합 주입).
    겹치지 않는 고정 창은 **시차가 창 크기에 가까워지면 급격히 손실되고 창을 넘으면
    아예 보이지 않는다.** 창 600초 기준 시차 300초 조합은 20회 중 13회만 관측되고
    (35% 손실), 시차 600초는 4회(80% 손실), 시차 1200초 이상은 0회였다. 창을 키우면
    범위는 늘지만 창 수가 줄어 통계가 부실해진다(6시간 창에서 창 190개).

    기준점 방식은 경계가 없다. A 가 난 시각마다 앞을 보므로 손실이 구조적으로 생기지
    않고, 시차 구간별로 세므로 **어느 지연에서 관계가 가장 강한지가 결과에 그대로 나온다.**
    A->B 와 B->A 를 따로 세기 때문에 방향도 지표에서 나온다(고정 창은 순서를 버린다).

    한 기준점에 대해 같은 축은 **처음 따라온 것만** 센다. 그래야 값이 "A 중 B 가 따라온
    비율"이 되어 해석이 확률로 고정된다.
    """
    width = lag_max / nbins
...
def overlap_counts(events: list, axis: str, min_open_s: int = 0):
    ...
    pairs = Counter()
    # 며칠에 걸쳐 나타났는지도 센다. 이게 없으면 **한 번의 대형 사건이 통계를 지배한다** —
    # 한 사건에 알림 종류가 N개면 쌍이 N개씩 곱해져 전부 유의 판정을 받는다.
    # 실측 2026-08-07: 통과 조합의 81%가 단 한 번의 사건에서 나왔다(수백 대 규모 호스트가
    # 몇 분 안에 함께 발화한 건).
    ...
    return {"pairs": pairs, "n_axis": n_axis,
            ...
            # 최대 하루가 전체에서 차지하는 비중. 한 사건이 통계를 지배했는지 가른다.
            # 일수만 보면 "겹침 252건이 4일에 몰린" 형태가 안 걸러진다 — 실측(2026-08-07)
            # 에서 그 4일 중 하루가 241건(96%)이었다. 이 지표는 발생 빈도를 벌하지 않는다:
            # 매일 100번씩 30일이면 최대 하루 비중 3% 로 통과한다(겹침/일수 비율로 하면
            # 그런 진짜 관계가 잘못 걸린다).
...
def permute_overlap(events: list, observed: dict, axis: str, rounds: int,
    ...
    """구간 겹침용 귀무모형 — **구간을 시간축에서 무작위로 옮긴다.**

    라벨을 섞는 방식을 먼저 썼다가 폐기했다. 구간 길이는 그 문제의 성질이다(어떤 문제는
    원래 오래 간다). 라벨을 섞으면 그 연결이 끊겨, 관측에서 길게 열리는 종류가 전부
    유의 판정을 받는다. 실측 2026-08-07: 검정 2,344개 중 **1,997개(85%)가 통과** —
    결과가 아니라 고장이다.

    재고 싶은 것은 "이 문제가 오래 열리는가"가 아니라 **"그 위에 나는 것이 특정 종류에
    쏠리는가"** 다. 그래서 구간의 길이와 라벨은 그대로 두고 **위치만** 무작위로 옮긴다.
    같은 길이의 구간이 아무 데나 놓였을 때 몇 개를 덮는지가 곧 우연의 기준선이 된다.
    """
...
def _cliques(pairs: list, min_conf: float = 0.8, min_days: int = 5, min_together: int = 5):
    ...
    leverage 로는 잡히지 않는 유형이다. 발생 자체가 드물면 관측·기대 빈도 차이가
    작아지기 때문이다. 실측(2026-08-07): 백엔드 상태 점검 3종이 **뜬 창 10개 전부에서
    셋이 함께** 떴는데 leverage 는 셔플 천장의 1배 경계였다. 반대로 회선 이중화 쌍은
    P(B|A)=0.85 인데 leverage 는 미미했다. 둘 다 병합 후보이므로 선별을 따로 둔다.

    min_days 를 두는 이유 — 하루에 몰린 10회는 한 번의 사건을 여러 번 센 것이다.
    """
...
def main():
    ...
    if a.load:
        ...
        # 실측 2026-08-07: 키워드 보강으로 미분류가 96% 줄었는데 --load 결과는 그대로였다.
